...
{
    public class GetAllBookingDoctorForUserHandler:IRequestHandler<GetAllBookingDoctorForUserQuery,ObjectResult>
    {
        private readonly IBookingDoctorRepository repository;
        private readonly IMapper mapper;
        private readonly ILogger<GetAllBookingDoctorForUserHandler> logger;
        private readonly ProfileService.ProfileServiceClient client;
        public GetAllBookingDoctorForUserHandler(IConfiguration configuration, IBookingDoctorRepository repository, IMapper mapper, ILogger<GetAllBookingDoctorForUserHandler> logger)
        {
        ...
        public async Task<ObjectResult> Handle(GetAllBookingDoctorForUserQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var bookingDoctor = await repository.GetAllBookingDoctor(x => x.BookingStatus == request.BookingStatus && x.UserID == Guid.Parse(request.UserID));
