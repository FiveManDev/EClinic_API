...
{
    public class GetAllBookingDoctorForAdHandler:IRequestHandler<GetAllBookingDoctorForAdQuery,ObjectResult>
    {
        private readonly IBookingDoctorRepository repository;
        private readonly IMapper mapper;
        private readonly ILogger<GetAllBookingDoctorForAdHandler> logger;
        private readonly ProfileService.ProfileServiceClient client;
        public GetAllBookingDoctorForAdHandler(IConfiguration configuration, IBookingDoctorRepository repository, IMapper mapper, ILogger<GetAllBookingDoctorForAdHandler> logger)
        {
        ...
        public async Task<ObjectResult> Handle(GetAllBookingDoctorForAdQuery request, CancellationToken cancellationToken)
        {
            try
            {
                var bookingDoctor = await repository.GetAllBookingDoctor(x => x.BookingStatus == request.BookingStatus);
