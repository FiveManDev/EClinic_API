...
{
    public class GetAllRoomOfSupporterHandler : IRequestHandler<GetAllRoomOfSupporterQuery, ObjectResult>
    {
        private readonly IRoomRepository roomRepository;
        private readonly IMapper mapper;
        private readonly ProfileService.ProfileServiceClient client;
        private readonly ILogger<GetAllRoomOfSupporterHandler> logger;

        public GetAllRoomOfSupporterHandler(IConfiguration configuration, IRoomRepository roomRepository, IMapper mapper, ILogger<GetAllRoomOfSupporterHandler> logger)
        {
        ...
        public async Task<ObjectResult> Handle(GetAllRoomOfSupporterQuery request, CancellationToken cancellationToken)
        {

            try
            {
                var UserID = Guid.Parse(request.UserID);
                var Rooms = await roomRepository.GetAllRoom(x => (x.ReceiverID == UserID || x.SenderID == UserID) && x.RoomTypeID == ConstantsData.SupporterRoomTypeID);
                if (Rooms.Count == 0)
                {
