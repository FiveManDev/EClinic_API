...
{
    public class CreateRefundAutoHandler : IRequestHandler<CreateRefundAutoCommand, ObjectResult>
    {
        ...
        private readonly ILogger<CreateRefundAutoHandler> logger;

        public CreateRefundAutoHandler(IPaymentRepository paymentRepository, IRefundRepository refundRepository, IMomoPayment momoPayment, IVNPayPayment vNPayPayment, ILogger<CreateRefundAutoHandler> logger)
        {
        ...
        public async Task<ObjectResult> Handle(CreateRefundAutoCommand request, CancellationToken cancellationToken)
        {
            try
            {
                var payment = await paymentRepository.GetPayment(x => x.BookingID == request.RefundModel.PaymentID);
                if (payment == null)
                {
